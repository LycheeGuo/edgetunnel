...
const SOCKS5白名单 = ['*tapecontent.net', '*cloudatacdn.com', '*loadshare.org', '*cdn-centaurus.com', 'scholar.google.com'];
...
export default {
    async fetch(request, env, ctx) {
        ...
        const proxyCtx = {
            ...
            parsedSocks5Address: {},
            学术反代IP: '',
            SOCKS5ProxyIP: '', // 存储从环境变量随机选中的那个代理
            SOCKS5白名单: env.GO2SOCKS5 ? await 整理成数组(env.GO2SOCKS5) : SOCKS5白名单
        };

        // 1. 处理普通反代IP
        if (env.PROXYIP) {
            const proxyIPs = await 整理成数组(env.PROXYIP);
            proxyCtx.反代IP = proxyIPs[Math.floor(Math.random() * proxyIPs.length)];
        } else {
            proxyCtx.反代IP = (request.cf.colo + '.PrOxYIp.CmLiUsSsS.nEt').toLowerCase();
        }

        // 2. 读取 ACADEMIC_PROXY 变量
        if (env.ACADEMIC_PROXY) {
            try {
                const academicIPs = await 整理成数组(env.ACADEMIC_PROXY);
                if (academicIPs.length > 0) {
                    proxyCtx.学术反代IP = academicIPs[Math.floor(Math.random() * academicIPs.length)];
                }
            } catch (e) { console.log('解析 ACADEMIC_PROXY 失败:', e); }
        }

        // 3. 读取 SOCKS5 变量 (自定义混用支持)
        if (env.SOCKS5) {
            try {
                const socks5IPs = await 整理成数组(env.SOCKS5);
                if (socks5IPs.length > 0) {
                    proxyCtx.SOCKS5ProxyIP = socks5IPs[Math.floor(Math.random() * socks5IPs.length)];
                }
            } catch (e) { console.log('解析 SOCKS5 变量失败:', e); }
        }
        ...
        if (!upgradeHeader || upgradeHeader !== 'websocket') {
            ...
            if (!管理员密码) return fetch(Pages静态页面 + '/noADMIN').then(r => new Response(r.body, { status: 404, headers: r.headers }));
            if (!env.KV) return fetch(Pages静态页面 + '/noKV').then(r => new Response(r.body, { status: 404, headers: r.headers }));
            ...
            if (访问路径 == 'admin' || 访问路径.startsWith('admin/')) {
                ...
                if (访问路径 === 'admin/log.json') {
                    ...
                    try {
                        ...
                        return new Response(JSON.stringify({ msg: '查询请求量失败: ' + err.message, error: err.message }, null, 2), { status: 500, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
                    }
                    ...
                    if (url.searchParams.get('url')) {
                        ...
                        try {
                            ...
                            return new Response(JSON.stringify({ msg: '验证优选API失败: ' + err.message, error: err.message }, null, 2), { status: 500, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
                        }
                    }
                    ...
                    else return new Response(JSON.stringify({ error: '缺少代理参数' }), { status: 400, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
                    return new Response(JSON.stringify(检测代理响应, null, 2), { status: 200, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
                }

                // 读取配置
                proxyCtx.config_JSON = await 读取config_JSON(env, host, userID, proxyCtx); 

                // KV 操作
                if (访问路径 === 'admin/init') {
                    try {
                        proxyCtx.config_JSON = await 读取config_JSON(env, host, userID, proxyCtx, true);
                        ctx.waitUntil(请求日志记录(env, request, 访问IP, 'Init_Config', proxyCtx.config_JSON));
                        proxyCtx.config_JSON.init = '配置已重置为默认值';
                        return new Response(JSON.stringify(proxyCtx.config_JSON, null, 2), { status: 200, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
                    } catch (err) {
                        return new Response(JSON.stringify({ msg: '配置重置失败: ' + err.message, error: err.message }, null, 2), { status: 500, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
                    }
                    ...
                    if (访问路径 === 'admin/config.json') {
                        try {
                            ...
                            ctx.waitUntil(请求日志记录(env, request, 访问IP, 'Save_Config', proxyCtx.config_JSON));
                        ...
                        try {
                            ...
                            ctx.waitUntil(请求日志记录(env, request, 访问IP, 'Save_Custom_IPs', proxyCtx.config_JSON));
                    ...
                    return new Response(JSON.stringify(proxyCtx.config_JSON, null, 2), { status: 200, headers: { 'Content-Type': 'application/json' } });
                } else if (区分大小写访问路径 === 'admin/ADD.txt') {
                    let 本地优选IP = await env.KV.get('ADD.txt') || 'null';
                    if (本地优选IP == 'null') 本地优选IP = (await 生成随机IP(request, proxyCtx.config_JSON.优选订阅生成.本地IP库.随机数量, proxyCtx.config_JSON.优选订阅生成.本地IP库.指定端口))[1];
                    return new Response(本地优选IP, { status: 200, headers: { 'Content-Type': 'text/plain;charset=utf-8', 'asn': request.cf.asn } });
                } else if (访问路径 === 'admin/cf.json') {
                    return new Response(JSON.stringify(request.cf, null, 2), { status: 200, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
                }

                ctx.waitUntil(请求日志记录(env, request, 访问IP, 'Admin_Login', proxyCtx.config_JSON));
            ...
            if (访问路径 === 'sub') {
                ...
                if (url.searchParams.get('token') === 订阅TOKEN) {
                    proxyCtx.config_JSON = await 读取config_JSON(env, host, userID, proxyCtx);
                    ctx.waitUntil(请求日志记录(env, request, 访问IP, 'Get_SUB', proxyCtx.config_JSON));
                    // 订阅生成逻辑 (直接嵌入简化版以适配)
                    const ua = UA.toLowerCase();
                    const expire = 4102329600;
                    const now = Date.now();
                    let pagesSum = 0, workersSum = 0, total = 0;
                    if (proxyCtx.config_JSON.CF.Usage.success) {
                        pagesSum = proxyCtx.config_JSON.CF.Usage.pages;
                        workersSum = proxyCtx.config_JSON.CF.Usage.workers;
                        total = 1024 * 100;
                    }
                    const responseHeaders = {
                        "content-type": "text/plain; charset=utf-8",
                        "Profile-Update-Interval": proxyCtx.config_JSON.优选订阅生成.SUBUpdateTime,
                        "Profile-web-page-url": url.protocol + '//' + url.host + '/admin',
                        "Subscription-Userinfo": `upload=${pagesSum}; download=${workersSum}; total=${total}; expire=${expire}`,
                        "Cache-Control": "no-store",
                    };
                    const 订阅类型 = url.searchParams.get('target') || (ua.includes('clash') ? 'clash' : 'mixed');
                    const 协议类型 = proxyCtx.config_JSON.协议类型;
                    let 订阅内容 = '';
                    const 节点路径 = proxyCtx.config_JSON.启用0RTT ? proxyCtx.config_JSON.PATH + '?ed=2560' : proxyCtx.config_JSON.PATH;
                    
                    if (订阅类型 === 'mixed' || !url.searchParams.has('sub')) {
                        const 完整优选列表 = (await 生成随机IP(request, proxyCtx.config_JSON.优选订阅生成.本地IP库.随机数量, proxyCtx.config_JSON.优选订阅生成.本地IP库.指定端口))[0];
                        // 生成节点
                        订阅内容 = 完整优选列表.map(原始地址 => {
                            const match = 原始地址.match(/^(\[[\da-fA-F:]+\]|[\d.]+|[a-zA-Z0-9.-]+)(?::(\d+))?(?:#(.+))?$/);
                            if (!match) return null;
                            const 节点地址 = match[1], 节点端口 = match[2] || "443";
                            const 随机国旗 = 国家国旗列表[Math.floor(Math.random() * 国家国旗列表.length)];
                            const 节点HOST = 随机替换通配符(host);
                            return `${协议类型}://${proxyCtx.config_JSON.UUID}@${节点地址}:${节点端口}?security=tls&type=${proxyCtx.config_JSON.传输协议}&host=${节点HOST}&sni=${节点HOST}&path=${encodeURIComponent(proxyCtx.config_JSON.随机路径 ? 随机路径() + 节点路径 : 节点路径)}&encryption=none${proxyCtx.config_JSON.跳过证书验证 ? '&allowInsecure=1' : ''}#${encodeURIComponent(随机国旗)}`;
                        }).filter(Boolean).join('\n');
                        订阅内容 = btoa(订阅内容);
                    }
            ...
            // WS 代理入口
            // 1. 解析 URL 参数中的代理设置 (写入 proxyCtx)
            await 反代参数获取(request, proxyCtx);
            
            // 2. 处理 WS 请求 (传入 proxyCtx)
            return await 处理WS请求(request, userID, proxyCtx);
        }
...
///////////////////////////////////////////////////////////////////////WS传输数据///////////////////////////////////////////////
async function 处理WS请求(request, yourUUID, proxyCtx) {
    ...
    readable.pipeTo(new WritableStream({
        async write(chunk) {
            ...
            if (判断是否是木马) {
                ...
                await forwardataTCP(hostname, port, rawClientData, serverSock, null, remoteConnWrapper, proxyCtx);
            } else {
                ...
                await forwardataTCP(hostname, port, rawData, serverSock, respHeader, remoteConnWrapper, proxyCtx);
            }
        },
    })).catch((err) => {});
...
// [核心修改] 转发 TCP 数据，支持请求级代理隔离和学术分流
async function forwardataTCP(host, portNum, rawData, ws, respHeader, remoteConnWrapper, proxyCtx) {
    // 复制当前请求的代理配置到局部变量，防止污染
    let 当前连接类型 = proxyCtx.启用SOCKS5反代;
    let 当前连接全局 = proxyCtx.启用SOCKS5全局反代;
    let 当前连接账号 = proxyCtx.我的SOCKS5账号;
    let 当前连接地址 = proxyCtx.parsedSocks5Address;

    // [学术分流逻辑] 仅当访问 scholar.google.com 时触发
    if (host.includes('scholar.google.com') && (proxyCtx.SOCKS5ProxyIP || proxyCtx.学术反代IP)) {
        try {
            // 优先使用 SOCKS5 变量中的 IP (已实现负载均衡), 其次使用 ACADEMIC_PROXY
            let 选中的代理 = proxyCtx.SOCKS5ProxyIP || proxyCtx.学术反代IP;
            ...
            if (选中的代理) {
                ...
                } else if (!proxyCtx.SOCKS5ProxyIP && proxyCtx.学术反代IP) {
                    // 兼容旧版 ACADEMIC_PROXY 默认 http
                    协议类型 = 'http';
                }

                // 覆盖当前连接的配置
                当前连接地址 = await 获取SOCKS5账号(待解析账号);
                当前连接类型 = 协议类型;
                当前连接全局 = true; 
                当前连接账号 = 待解析账号;
                // console.log(`[学术分流] 命中: ${host}, 使用代理: ${待解析账号}`);
            }
        } catch (e) {
    ...
    console.log(JSON.stringify({ 目标: host, 端口: portNum, 代理: 当前连接类型, 全局: 当前连接全局 }));
    ...
    async function connecttoPry() {
        let newSocket;
        // 使用局部变量 当前连接类型/地址 进行连接
        if (当前连接类型 === 'socks5') {
            newSocket = await socks5Connect(host, portNum, rawData, 当前连接地址);
        } else if (当前连接类型 === 'http' || 当前连接类型 === 'https') {
            newSocket = await httpConnect(host, portNum, rawData, 当前连接地址);
        } else {
            // 直连或使用 ProxyIP
            try {
                const [反代IP地址, 反代IP端口] = await 解析地址端口(proxyCtx.反代IP);
...
async function socks5Connect(targetHost, targetPort, initialData, parsedAddr) {
...
async function httpConnect(targetHost, targetPort, initialData, parsedAddr) {
    const { username, password, hostname, port } = parsedAddr;
    const socket = connect({ hostname, port });
    const writer = socket.writable.getWriter();
    const reader = socket.readable.getReader();
...
async function 读取config_JSON(env, hostname, userID, ctx, 重置配置 = false) {
    ...
    const 默认配置JSON = {
        ...
        CF: { Email: null, GlobalAPIKey: null, AccountID: null, APIToken: null, Usage: { success: false, pages: 0, workers: 0, total: 0 } }
    };
    ...
    config_JSON.PATH = ctx.启用SOCKS5反代 ? ('/' + ctx.启用SOCKS5反代 + (ctx.启用SOCKS5全局反代 ? '://' : '=') + ctx.我的SOCKS5账号) : (ctx.反代IP === 'auto' ? '/' : `/proxyip=${ctx.反代IP}`);
...
async function 反代参数获取(request, ctx) {
    const url = new URL(request.url);
    const { pathname, searchParams } = url;
    const pathLower = pathname.toLowerCase();

    ctx.我的SOCKS5账号 = searchParams.get('socks5') || searchParams.get('http') || null;
    ctx.启用SOCKS5全局反代 = searchParams.has('globalproxy') || false;

    const proxyMatch = pathLower.match(/\/(proxyip[.=]|pyip=|ip=)(.+)/);
    if (searchParams.has('proxyip')) {
        const 路参IP = searchParams.get('proxyip');
        ctx.反代IP = 路参IP.includes(',') ? 路参IP.split(',')[Math.floor(Math.random() * 路参IP.split(',').length)] : 路参IP;
        return;
    } else if (proxyMatch) {
    ...
    if ((socksMatch = pathname.match(/\/(socks5?|http):\/?\/?(.+)/i))) {
        ctx.启用SOCKS5反代 = socksMatch[1].toLowerCase() === 'http' ? 'http' : 'socks5';
        ctx.我的SOCKS5账号 = socksMatch[2].split('#')[0];
        ctx.启用SOCKS5全局反代 = true;
        if (ctx.我的SOCKS5账号.includes('@')) {
            const atIndex = ctx.我的SOCKS5账号.lastIndexOf('@');
            let userPassword = ctx.我的SOCKS5账号.substring(0, atIndex).replaceAll('%3D', '=');
            if (/^(?:[A-Z0-9+/]{4})*(?:[A-Z0-9+/]{2}==|[A-Z0-9+/]{3}=)?$/i.test(userPassword) && !userPassword.includes(':')) {
                userPassword = atob(userPassword);
            }
            ctx.我的SOCKS5账号 = `${userPassword}@${ctx.我的SOCKS5账号.substring(atIndex + 1)}`;
        }
    } else if ((socksMatch = pathname.match(/\/(g?s5|socks5|g?http)=(.+)/i))) {
        const type = socksMatch[1].toLowerCase();
        ctx.我的SOCKS5账号 = socksMatch[2];
        ctx.启用SOCKS5反代 = type.includes('http') ? 'http' : 'socks5';
        ctx.启用SOCKS5全局反代 = type.startsWith('g') || ctx.启用SOCKS5全局反代;
    }

    if (ctx.我的SOCKS5账号) {
        try {
            ctx.parsedSocks5Address = await 获取SOCKS5账号(ctx.我的SOCKS5账号);
            ctx.启用SOCKS5反代 = searchParams.get('http') ? 'http' : ctx.启用SOCKS5反代;
        } catch (err) {
            ctx.启用SOCKS5反代 = null;
        }
    } else ctx.启用SOCKS5反代 = null;
}
...
async function 获取SOCKS5账号(address) {
    ...
    if (isNaN(port)) throw new Error('无效的 SOCKS 地址格式');
...
async function getCloudflareUsage(Email, GlobalAPIKey, AccountID, APIToken) {
    ...
    try {
        ...
        return { success: true, pages, workers, total: pages + workers };
    } catch (error) {
...
async function SOCKS5可用性验证(代理协议 = 'socks5', 代理参数) {
    ...
    try { 
        var parsedSocks5Address = await 获取SOCKS5账号(代理参数); 
    } catch (err) { 
    ...
    try {
        ...
        const tcpSocket = 代理协议 == 'socks5' ? await socks5Connect('check.socks5.090227.xyz', 80, initialData, parsedSocks5Address) : await httpConnect('check.socks5.090227.xyz', 80, initialData, parsedSocks5Address);
